...
export function SpaceBackground() {
  ...
  useEffect(() => {
    ...
    const sunGlow1 = new THREE.Sprite(
      new THREE.SpriteMaterial({
    ...
    for (const pd of PLANET_DATA) {
      ...
      const entry: PlanetEntry = {
        ...
        mesh,
        speed: pd.speed,
        selfSpin: 0.008 + Math.random() * 0.012,
      }
    ...
    function buildRocket(): THREE.Group {
      ...
      for (let i = 0; i < 3; i++) {
        const fin = new THREE.Mesh(
          new THREE.BoxGeometry(0.05, 0.38, 0.3),
          finMat
        )
    ...
    function respawnRocket(r: Rocket) {
      ...
      r.group.quaternion.setFromUnitVectors(
        new THREE.Vector3(0, 1, 0),
    ...
    const rockets: Rocket[] = Array.from({ length: 5 }, (_, i) =>
      initRocket(i < 3)
    )
    ...
    function buildSatellite(): THREE.Group {
      ...
      // Solar panel right
      const panelRight = new THREE.Mesh(
        new THREE.BoxGeometry(1.2, 0.02, 0.5),
        panelMat
      )
    ...
    function buildAsteroid(): THREE.Mesh {
      ...
      const grays = [
        0x888888, 0x999999, 0x777777, 0xaaaaaa, 0x666666, 0x909090, 0x7a7a7a,
      ]
    ...
    const asteroids: Asteroid[] = Array.from({ length: 18 }, (_, i) =>
      initAsteroid(i < 14)
    )
    ...
    function animate() {
      ...
      for (const r of rockets) {
        ...
        const flicker =
          0.9 + Math.sin(r.flamePhase) * 0.1 + Math.random() * 0.06
        const c = r.group.children
        if (c[r.flameIdx[0]])
          c[r.flameIdx[0]].scale.set(
            flicker,
            0.9 + Math.random() * 0.22,
            flicker
          )
        if (c[r.flameIdx[1]])
          c[r.flameIdx[1]].scale.set(
            flicker,
  ...
  return (
    <div ref={mountRef} className="pointer-events-none fixed inset-0 -z-10" />
  )
}
